...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef* pdev, uint8_t epnum)
{
  ...
  uint16_t true_dim = haudio->buffer_length;     // Get the size of the buffer
  uint16_t packet_dim = haudio->paketDimension;  // Get the packet size
  uint16_t channels = haudio->channels;          // Get the number of channels
  length_usb_pck = packet_dim;                   // Set the length of USB packet

  // Reset timeout counter
  haudio->timeout = 0;

  // Check if the endpoint number matches AUDIO_IN_EP
  if (epnum == (AUDIO_IN_EP & 0x7F)) {
    ...
    // If the state is STATE_USB_BUFFER_WRITE_STARTED, start transmitting the audio data
      ...
      // Adjust the length of USB packet based on available space in the buffer
    ...
    // If the state is not STATE_USB_BUFFER_WRITE_STARTED, transmit dummy data
    else {
      USBD_LL_Transmit(pdev, AUDIO_IN_EP, IsocInBuffDummy, length_usb_pck);
    }
  }
